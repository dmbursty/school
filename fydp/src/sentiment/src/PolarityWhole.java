...
public class PolarityWhole {
    ...
    void evaluate(int fold,
        ...
        while (sc.hasNextLine())
        {
            ...
            Classification classification
                    = compiledClassifier.classify(review);
    ...
    void readData() throws IOException {
        mPosReviewLists = readData(new File(mPolarityDir, "good"));
        mNegReviewLists = readData(new File(mPolarityDir, "bad"));
    ...
    List<List<String>> readData(File polarfile) throws IOException {
        List<List<String>> foldReviewLists = new ArrayList<List<String>>();
        for (int i = 0; i < NUM_FOLDS; ++i)
            foldReviewLists.add(new ArrayList<String>());

        StringBuilder sb = new StringBuilder();
        BufferedReader br = new BufferedReader(new FileReader(polarfile));
        int i = 0;
        String line;
        while ( (line = br.readLine()) != null)
        {
            if (line.startsWith("<!-- BOUNDARY -->"))
            {
               ...
               i++;
               addTokens(review);
               foldReviewLists.get( i % NUM_FOLDS ).add(review);
