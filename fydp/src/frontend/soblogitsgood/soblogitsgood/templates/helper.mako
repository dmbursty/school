...
<%def name="printResult(r)">
  <h3 class="resultLink">
    <a href="${r['link']}">${boldTerm(r['title'])}</a><br/>
  </h3>
  <p class="result">
    % if r['sentiment'] >= 0.5:
      <span class="good">Sentiment: Good</span><br/>
    % else:
      <span class="bad">Sentiment: Bad</span><br/>
    % endif
...
## This method should be called *last* in the new Highcharts.Chart({ }) 
## hash definition. Look at analysis.mako for example. 
<%def name="sort(results)">
  ...
  context.write("xAxis: { categories: " + str(axis) + "}, \n")
  context.write("series: [{ name: 'Results', color: '#363636', \n")
  context.write("data: " + str(buckets) + "}]")
  %>
</%def>
